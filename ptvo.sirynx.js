...
const device = {
    ...
    exposes: [
	  e.contact().withEndpoint('l1').withDescription('Tamper'),
      e.contact().withEndpoint('l2').withDescription('Battery Failure'),
	  e.contact().withEndpoint('l3').withDescription('Power Failure'),
	  e.switch().withEndpoint('l4').withDescription('Intrusion Siren'),
      e.switch().withEndpoint('l5').withDescription('Discontinuous Modulation')
],
    meta: {
        multiEndpoint: true,
        binaryEndpoints: {'l1': 'contact', 'l2': 'contact', 'l3': 'contact', 'l4': 'switch', 'l5': 'switch'}, 
    },
    endpoint: (device) => {
        return {
